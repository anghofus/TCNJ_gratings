...
class Motor():
    def __init__(self, port):
        ...
        for axis in (1,2):
            ...
            self.write_command(str(axis)+'VA'+'0.005')  # Sets velocity: this will be important for printing a grating 
...
def open_images():
    ...
    print("you are showing " +str(names[0])+ " please make sure that the images are read in the order you want them to be")
    slm=SLM_window(root)
...
def movement1():
    ...
    motor.move_rel(1,-0.12)
    motor.close()
    print("move to next matrix please")
    shutter.close()
...
btn7.pack(side="left")
btn3=tk.Button(root,text="print 4 lines", bd="5", command= movement1)
btn3.pack(side="left")
#btn3=tk.Button(root,text="print -", bd="5", command= movement2)
#btn3.pack(side="left")
btn6=tk.Button(root,text="new matrix", bd="5", command= new_matrix)
btn6.pack(side="left")
